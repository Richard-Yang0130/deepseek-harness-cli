...
rem   --resume: read ~/.dsh-cli/resume.txt (legacy ~/.dsh-cc/resume.txt as
rem             fallback) and feed it to the TUI as DSH_CLI_RESUME_SESSION —
...
if /i "%~1"=="--resume" (
  if exist "%USERPROFILE%\.dsh-cli\resume.txt" (
    ...
    set /p DSH_CLI_RESUME_SESSION=<"%USERPROFILE%\.dsh-cc\resume.txt"
  )
  if defined DSH_CLI_RESUME_SESSION set "DSH_CC_RESUME_SESSION=%DSH_CLI_RESUME_SESSION%"
) else (
  set "ARGS=%ARGS% "%~1""
)
shift
