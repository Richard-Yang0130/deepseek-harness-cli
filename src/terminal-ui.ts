/** Terminal presentation helpers shared by the runner and unit tests. */
...
function textOf(blocks: readonly ContentBlock[]): string {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'tool-result') output.push(textOf(block.content))
  }
  return output.join('')
}
...
export function parseTerminalInput(input: string): TerminalInput {
  ...
  if (text === '/resume') return { kind: 'resume', sessionId: '' }
  if (text.startsWith('/resume ')) return { kind: 'resume', sessionId: text.slice('/resume '.length).trim() }
  ...
  if (text === '/rename') return { kind: 'rename', title: '' }
  if (text.startsWith('/rename ')) return { kind: 'rename', title: text.slice('/rename '.length).trim() }
  if (text === '/workspace') return { kind: 'workspace', path: '' }
  if (text.startsWith('/workspace ')) return { kind: 'workspace', path: text.slice('/workspace '.length).trim() }
  return { kind: 'prompt', text }
}

function questionPrompt(question: AskUserQuestionItem): string {
...
export async function answerQuestions(
  ...
  for (const question of questions) {
    ...
    const selected = [...new Set(indexes)].map(index => (options[index] as { label: string }).label)
