 ...
 * DSH_CLI_THEME (built-in or user theme name) > persisted `/theme` choice >
 ...
 * validates the name, persists the choice to ~/.dsh-cli/theme.json and hot
 * swaps the palette (and the module-level mirror) immediately.
 */

// User themes (~/.dsh-cli/themes/<name>.json) resolve through this registry,
...
export function ThemeProvider({
  ...
  const [forcedValid] = useState<boolean>(() => {
    ...
    console.warn(
      `[dsh-tui] theme "${forced}" not found (built-ins: auto, light, dark, dark-ansi; user themes: ~/.dsh-cli/themes/*.json); falling back to auto-detection`,
    )
    return false
  })
  ...
  const setTheme = React.useCallback(
    (name: string): boolean => {
      ...
      if (!writeThemePref(name)) {
        console.warn('[dsh-tui] failed to write ~/.dsh-cli/theme.json')
        return false
      }
      setActive(name)
