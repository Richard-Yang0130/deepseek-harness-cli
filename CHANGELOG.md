# Changelog

## 0.1.2 — 2026-08-14

- Make npm's `github:` installer package the committed runtime instead of leaving a dangling link to its temporary clone.

## 0.1.1 — 2026-08-14

- Ship compiled runtime files in Git so `npm install -g github:…` does not depend on development tools during installation.

## 0.1.0 — 2026-08-14

- Initial standalone terminal release.
- Automatic isolated dsh profile bootstrap.
- Claude Code-style Ink shell with downward `/` menu and `@` subagent menu.
- Eight-row wide DeepSeek whale header and narrow-terminal fallback.
- Operational mapping for sessions, models, credentials, presets, settings, plugins, jobs, attachments, exports, statistics, and feedback.
- Harness-native approvals, questions, commands, skills, tools, subagents, goals, plans, and workflows.
- Privacy scan and sanitized screenshot asset.
